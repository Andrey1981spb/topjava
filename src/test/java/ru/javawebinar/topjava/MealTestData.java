...
public class MealTestData<M> {
    ...
    public static <T> void assertMatchMeal(T actual, T expected) {
        assertThat(actual).isEqualToIgnoringGivenFields(expected, "user");
    }

    public static <T> void assertMatchMeal(Iterable<T> actual, T... expected) {
    ...
    public static <T> ResultMatcher contentJsonMeal(T... expected) {
        return result -> assertMatchMeal(readListFromJsonMvcResult(result, expected[0].getClass()), List.of(expected));
    }

    public static ResultMatcher contentJsonMeal(Meal expected) {
        return result -> assertMatchMeal(readFromJsonMvcResult(result, Meal.class), expected);
    }
